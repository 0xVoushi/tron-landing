  ...
  return (
    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center px-6 pointer-events-none [&_a]:pointer-events-auto [&_button]:pointer-events-auto [&_h1]:pointer-events-auto [&_p]:pointer-events-auto [&_span]:pointer-events-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-12 w-full max-w-3xl">
        {STATS.map((stat) => (
          <div key={stat.label} className="rounded-lg p-4 flex flex-col items-center text-center">
            <p className="font-rubik font-bold text-[24px] md:text-[28px] text-white leading-none mb-1">{stat.value}</p>
            <p className="font-rubik font-semibold text-[12px] text-white mb-0.5 bg-dark-hard px-2 py-0.5 rounded-lg w-fit">{stat.label}</p>
            <p className="font-rubik text-[11px] text-white bg-dark-hard px-2 py-0.5 rounded-lg w-fit">{stat.sub}</p>
          </div>
        ))}
      </div>
    </div>
